...
class aafwEntityStoreBase extends aafwObject {
  ...
  protected static $_SQLLogs       = array ();
  ...
  public function __construct ( $params = null ) {
    ...
    if ( !$this->_Catalog ) {
      ...
      if ( !isset ( self::$_CatalogList[$table_name] ) ) self::$_CatalogList[$table_name] = $this->_StoreRead->getTableInfo ( $table_name );
  ...
  public function find ( $filter ) {
    ...
    if ( $for_update || $in_trans ) $res = $this->_StoreMaster->execute ( $sql );
  ...
  public function createFilters ( $filter ) {
    ...
    $tmp = array ();
    if ( $conditions || $where ) {
      if ( $conditions ) foreach ( $this->createWhereFromConditions ( $conditions ) as $elm )       $tmp[] = $elm;
            if ( $where )      $tmp[] = '( ' . $this->createWhereFromParams ( $where, $where_params ) . ' )';
    }
  ...
  public function save ( $obj ) {
    if ( $this->isExists ( $obj ) ) {
      $sql =  $this->createUpdateSQL ( $obj ) ;
      self::$_SQLLogs[] = $sql;
      $this->_StoreMaster->execute ( $sql );
    }
    else {
      $this->_StoreMaster->execute ( $this->createInsertSQL ( $obj ) );
  ...
  public function deleteLogical ( $obj ) {
    $sql = "UPDATE  $this->_TableName SET $this->_DeleteFlagName = 1 " . $this->createThisWhere ( $obj );
    self::$_SQLLogs[] = $sql;
    return $this->_StoreMaster->execute ( $sql );
  }
  ...
  public function count ( $filter, $col = null ) {
    if ( !$this->_StoreRead ) throw new Exception ( 'DBがありません' );
        $join          = null;
        if ( is_array ( $filter ) ) {
            $join        = $filter['join'];
            unset ( $filter['order'] );
  ...
  public function doFunctions ( $fname, $col_name , $conditions = null ) {
    $uniq = uniqid();
    $sql = "SELECT $fname(IFNULL(`$col_name`,0)) as $uniq FROM $this->_TableName " . $this->createFilters ( $conditions );
    self::$_SQLLogs[] = $sql;
    $row = $this->_StoreRead->fetch($this->_StoreRead->execute($sql));
    return $row[$uniq];
  }
