...
class aafwPhysicalEntityBase{
	...
	public function __construct( $db_group_id = null, $db_id = null ){
		if ( is_object ( $db_group_id ) ) {
			if ( $db_group_id instanceof DBInterface ) {
				$this->db_group_id = '__Default';
				$this->db_list = array (
					'w' => $db_group_id,
					'r' => $db_group_id,
				);
				...
				throw new aafwExceptin ( '引数が不正です' );
			}
		}
		else {
		...
		if(!isset($this->table_name)) {
			$this->table_name = $this->convertName ( get_class($this) );
			if ( !preg_match ( '#s$#', $this->table_name ) ) {
				$this->table_name .= 's';
			}
		}
			...
			if(is_array($temp)){
				...
				if( $this->convertName( $column ) == $class_name and ( !isset( $this->values[$column] ) || ( $order || $page ) ) ){
					...
					foreach($key_array as $column_related_obj => $column_base_obj){
						$conditions[$column_related_obj] = (is_null($this->values[$column_base_obj]) ? '' : $this->values[$column_base_obj]);
					}
	...
	public function isSaved(){
		...
		$conditions = array();
		foreach($this->key_list as $column){
			if ( is_null( $this->values[$column] ) ) continue;
	...
	public function find($find_arg = NULL, $for_update = false, $no_fetch = false ){
		...
		if(!is_array($find_arg)){
			...
			array_key_exists('conditions', $find_arg) or
			array_key_exists('order'     , $find_arg) or
			array_key_exists('limit'     , $find_arg) or
			array_key_exists('offset'    , $find_arg) or
			array_key_exists('where'     , $find_arg) or
			array_key_exists('pager'     , $find_arg) or
			array_key_exists('callback'  , $find_arg)
		){
		...
		if(!is_array($conditions) and isset($conditions) and count($this->key_list) == 1 and count($ret) != 0){
			return $this;
		}else{
			return $ret;
		}
	...
	public static function finds($find_arg = NULL, $for_update = false, $no_fetch = false, $db_group_id = null, $db_id = null){
		...
		$subClassObj = new $subClass($db_group_id, $db_id);
		return $subClassObj->find($find_arg, $for_update, $no_fetch);
	}

	public static function counts ( $find_arg = NULL) {
		$subClass = get_called_class();
		if($subClass == __CLASS__) return false;
		$subClassObj = new $subClass($db_group_id, $db_id);
		return $subClassObj->count ( $find_arg );
	}
	...
	public function count( $conditions = NULL, $where = NULL, $where_params = NULL ){
		$sql = "SELECT COUNT(*) cnt FROM " . $this->table_name . ' ' . $this->createConditionWhere($conditions, $where, $where_params);
		$row = $this->db_read->fetch($this->db_read->execute($sql));
		return $row[cnt];
	}
	...
	public function loadCSVTo( $data, $db_group = null ){
		...
		for ( $i = 0; $i < mb_strlen( $data ); $i++ ) {
			...
			if ( $quote ){
				if     ( $quote == $char )   $quote = '';
				elseif ( $char  == '\\'  ) { $buf .= $char . $data[$i+1]; $i++; }
				else                         $buf .= $char;
			}
			else {
				...
				elseif ( $char == ',' )                 { $columns[] = $buf; $buf = ''; }
				else                                    { $buf .= $char; }
			}
		}
		// 最後が改行じゃない場合の対応
		if ( $buf ) { $columns[] = $buf; $rows[] = $columns; }
