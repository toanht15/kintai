...
abstract class aafwPOSTActionBase extends aafwActionBase {
  ...
  public function doService ( ) {
    $this->ActionForm =  $this->SESSION['ActionContainer'][$this->ContainerName]['Result'];
    $ret = $this->doAction();
    if ( $this->Data['saved'] ) {
      $this->SESSION['ActionContainer'][$this->ContainerName] = null;
    } else {
      $this->SESSION['ActionContainer'][$this->ContainerName]['Result'] = $this->POST;
    }
  ...
  public function run () {
    ...
    $this->SESSION['ActionContainer'][$this->ContainerName]['ValidateError'] = null;
    ...
    if( $this->Validator ){
      ...
      if( !$ret ){
        ...
        $this->SESSION['ActionContainer'][$this->ContainerName]['ValidateError'] = $this->POST;
        $this->SESSION['ActionContainer'][$this->ContainerName]['Errors']        = $this->Validator;
      ...
      if( in_array( 'validate', $methods ) ){
        if ( $this->ValidatorDefinition ) {
          ...
          if ( !$validator->validate ( $this->REQUEST ) ){
            if( count( $this->_Plugins['Finally'] ) )
              foreach( $this->_Plugins['Finally'] as $c ) $c->doService();
            $this->SESSION['ActionContainer'][$this->ContainerName]['ValidateError'] = $this->POST;
            $this->SESSION['ActionContainer'][$this->ContainerName]['Errors']        = $validator;
            return $this->getFormURL();
          }
        }
        ...
        if( !$ret )  {
          ...
          $this->SESSION['ActionContainer'][$this->ContainerName]['ValidateError'] = $this->POST;
          $this->SESSION['ActionContainer'][$this->ContainerName]['Errors']        = $this->Errors;
          return $this->getFormURL();
        }
      }
