...
class aafwController {
   ...
   */
   public function run(){
     ...
     require_once('session' . DIRECTORY_SEPARATOR . $sessionClass . '.php');
     ...
     try {
       ...
       if( !is_file( $action['path'] ) )
         throw new Exception( 'actionの指定に誤りが有ります:' . $action['path'] );
       ...
       if( count( $f ) ){
         ...
         foreach( $f as $key => $value ) {
           ...
           $files[$key] = array();
           $files[$key]['name'] = $tmpdir . '/' . preg_replace( '#[/ 　]#', '', $value['name'] );
           if( $value['error'] ){
  ...
  private function renderErrorPage( $e ){
    ...
    if( is_file( $path ) ){
      ...
      if ( !$e instanceof aafwException ) $e = new aafwException($e);
    ...
    if( self::$Config->Debug  ) {
      print "<pre>";
      print "メッセージ\n";
      var_dump ( $e instanceof aafwException ? $e->getErrorMessage () : $e->getMessage () );
      print "\n\n";
      print "スタックトレース\n";
      var_dump( $e->getTrace() );
      print "\n\n";
