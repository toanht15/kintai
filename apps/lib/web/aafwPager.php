...
class aafwPager extends aafwWidgetBase {
  ...
  public function doService ( $params ) {
    ...
    if ( !$params['PageLimit'] )           $params['PageLimit']   = 10;
  ...
  public function calcStart ( $current, $page_limit, $total_page ) {
    ...
    if     ( $start >= $page_limit && $end - $start < $page_limit ) return $total_page - $page_limit + 1;
    elseif ( $start > 0  )                                          return $start;
    else                                                            return 1;
  }
  ...
  public function calcEnd ( $current, $page_limit, $total_page ) {
    ...
    elseif ( $end <=  $page_limit && $page_limit < $total_page) return $page_limit;
    else                            return $end;
  }
  ...
  public function analyzeURLBase ( $url ) {
    ...
    sort ( $keys );
    foreach ( $keys as $key ) {
      if ( $key == 'p' ) $query[] = urlencode ( $key ) . '=' . $tmp[$key];
      else               $query[] = urlencode ( $key ) . '=' . urlencode ( $tmp[$key] );
    }
